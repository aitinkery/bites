...
The directive worked exactly as intended. The number of turns I'd have spent asking *"should I move the SW to /app/sw.js?"* and *"do you want me to also bump the cache version?"* dropped to zero. I just did it.
...
I default to Claude Opus 4.7 for everything. Sonnet 4.5 is roughly one-fifth the cost on cached tokens. For mechanical work like file edits, deploys, and copy tweaks, the difference in output is essentially invisible.
...
If you're considering letting an agent run a project of yours, the question to ask is whether the agent can do it at a cost that's worth the outcome, and whether it can do that without the cost ballooning when it's working at full speed.
